// Hidden Triples - IMPLEMENTED
// When three candidates appear only in three cells of a unit,
...
export const hiddenTriples: Technique = {
  id: "hidden-triples",
  name: "Hidden Triples",
  difficulty: "hard",
  implemented: true,
  description:
    "Three candidates appear only in three cells of a unit. Other candidates can be eliminated from those cells.",
  ...
  apply(grid: SudokuGrid): TechniqueResult | null {
    ...
    for (const unit of units) {
      ...
      for (let d = 1; d <= 9; d++) {
        ...
        // Only consider digits that appear in 2 or 3 cells
        if (positions.length >= 2 && positions.length <= 3) {
          digitPositions.set(digit, positions);
        }
      }

      // Need at least 3 digits to form a hidden triple
      ...
      for (let i = 0; i < eligibleDigits.length; i++) {
        for (let j = i + 1; j < eligibleDigits.length; j++) {
          for (let k = j + 1; k < eligibleDigits.length; k++) {
            ...
            // If exactly 3 cells contain these 3 digits, we have a hidden triple
            if (allPositions.size === 3) {
              const tripleCells = Array.from(allPositions);
              const tripleDigits = [digit1, digit2, digit3];

              // Find eliminations - remove other candidates from the triple cells
              ...
              for (const cellIdx of tripleCells) {
                const cell = grid[cellIdx];
              ...
              if (eliminations.length > 0) {
                ...
                const cellDescriptions = tripleCells
                  .map((idx) => `R${grid[idx].row + 1}C${grid[idx].col + 1}`)
                  .join(", ");

                return {
                  techniqueId: "hidden-triples",
                  placements: [],
                  eliminations,
                  description: `Hidden Triple {${tripleDigits.join(",")}} in ${cellDescriptions} in ${unitName}`,
                };
              }
            }
          }
